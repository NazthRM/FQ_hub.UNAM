...
# ==========================================
# 2. CARGA DE DATOS (Conectando con data_loader.py)
# ==========================================
...
if df_horarios.empty or df_relaciones.empty:
    st.error(
        "No se pudo cargar la base de datos de horarios. Revisa la ruta en data_loader.py"
    )
else:
    ...
    if vista_actual == "Buscador de Grupos":
        ...
        if filtro_semestre:
            # Aquí aplicamos las nuevas categorías limpias
            relaciones_filtradas = relaciones_filtradas[
                (relaciones_filtradas["Semestre"].isin(filtro_semestre))
                | (
                    relaciones_filtradas["Caracter"].isin(
                        [
                            "Optativa Disciplinaria",
                            "Inorgánica de Elección",
                            "Sociohumanística",
                        ]
                    )
                )
    ...
    # --- VISTA 2: PREDICCIÓN DE CUPO ---
        ...
        if not st.session_state.grupos_guardados:
            st.info(
                "Aún no has guardado ningún grupo. Utiliza el **Buscador de Grupos** para seleccionar las materias de tu interés."
            )
        else:
